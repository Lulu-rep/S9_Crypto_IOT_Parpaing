  ...
  uint8_t InMAC)
{
      ...
      (pOutDecrement != NULL))
  {
    ...
    pStSafeA->InOutBuffer.LV.Data[2] = (uint8_t)(offset & 0x00FFU);
    pStSafeA->InOutBuffer.LV.Data[3] = (uint8_t)((offset & 0xFF00U)>>8);

    /* Amount */
    uint32_t amount = (uint32_t)SWAP4BYTES(InAmount);
    pStSafeA->InOutBuffer.LV.Data[4] = (uint8_t)(amount & 0x000000FFU); 
    pStSafeA->InOutBuffer.LV.Data[5] = (uint8_t)((amount & 0x0000FF00U)>>8);
    pStSafeA->InOutBuffer.LV.Data[6] = (uint8_t)((amount & 0x00FF0000U)>>16);
    pStSafeA->InOutBuffer.LV.Data[7] = (uint8_t)((amount & 0xFF000000U)>>24);
    ...
    if (status_code == STSAFEA_OK)
    {
      ...
      if (status_code == STSAFEA_OK)
      {
        ...
        pOutDecrement->OneWayCounter = (((uint32_t)(pStSafeA->InOutBuffer.LV.Data[0] & 0x000000FFU)) | ((uint32_t)((pStSafeA->InOutBuffer.LV.Data[1]) & 0x000000FFU)<<8) 
                                       | ((uint32_t)(pStSafeA->InOutBuffer.LV.Data[2] & 0x000000FFU) <<16) | ((uint32_t)(pStSafeA->InOutBuffer.LV.Data[3]& 0x000000FFU) <<24)) ;
  ...
  uint8_t InMAC)
{
  ...
  if (IS_STSAFEA_HANDLER_VALID_PTR(pStSafeA))
  {
    ...
    pStSafeA->InOutBuffer.LV.Data[4] = (uint8_t)(length & 0x00FFU);
    pStSafeA->InOutBuffer.LV.Data[5] = (uint8_t)((length & 0xFF00U)>>8);
  ...
  uint8_t InMAC)
{
      ...
      (pOutPointReprensentationId != NULL))
  {
    ...
    pStSafeA->InOutBuffer.LV.Data[2] = (uint8_t)(tmp_use_limit & 0x00FFU);
    pStSafeA->InOutBuffer.LV.Data[3] = (uint8_t)((tmp_use_limit & 0xFF00U) >>8);
    ...
    pStSafeA->InOutBuffer.LV.Data[4] = (uint8_t)(tmp_use_limit & 0x00FFU);
    pStSafeA->InOutBuffer.LV.Data[5] = (uint8_t)((tmp_use_limit & 0xFF00U) >>8);
    ...
    pStSafeA->InOutBuffer.LV.Data[6] = (uint8_t)(tmp_use_limit & 0x00FFU);
    pStSafeA->InOutBuffer.LV.Data[7] = (uint8_t)((tmp_use_limit & 0xFF00U) >>8);
    /* Update Frame Length */
    pStSafeA->InOutBuffer.LV.Length = tmp_len;
  ...
  uint8_t InHostEncryption)
{
      ...
      IS_STSAFEA_CONDITIONAL_LVBUFFER_VALID_PTR(pOutSignS))
  {
    ...
    for(i= 0; i<DigestLength ; i++)
    {
      pStSafeA->InOutBuffer.LV.Data[3 + i] = pInDigest[i];
    }
#else
    
    for(i =0; i< ((InDigestType == STSAFEA_SHA_384) ? STSAFEA_SHA_384_LENGTH : STSAFEA_SHA_256_LENGTH) ; i++)
    {
      pStSafeA->InOutBuffer.LV.Data[3 + i] = pInDigest[i];
    }
  ...
  uint8_t InMAC)
{
      ...
      (pOutVerifyPassword != NULL))
  {
    ...
    pStSafeA->InOutBuffer.LV.Data[0] = pInPassword[0];
    pStSafeA->InOutBuffer.LV.Data[1] = pInPassword[1];
