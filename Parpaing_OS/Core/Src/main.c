...
int main(void)
{
  ...
  if (HAL_I2C_Master_Transmit(&hi2c2, VEML6030_ADDR, config_data, 3, 100) != HAL_OK) {
      log_printf("I2C Error: Sensor not found!\r\n");
      HAL_GPIO_WritePin(GPIOH, LED_RED_Pin, GPIO_PIN_SET);
  } else {
      log_printf("VEML6030 initialized and waking up...\r\n");
  }

  // CRITICAL: The sensor needs time to perform the first conversion
  HAL_Delay(150);
  /* USER CODE END 2 */
    ...
    while (1)
    {
    	uint8_t reg = VEML6030_REG_ALS_DATA;
    	    ...
    	    if (status == HAL_OK) {
    	        ...
    	        log_printf("Lux: %.2f (Raw: %u)\r\n", lux_value, raw_als_data);
    	    }
    	    else {
    	        log_printf("I2C Read Failed. Status: %d\r\n", status);
    	    }
